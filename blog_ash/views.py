...
def index(request):
    ...
    return render(request, 'blog_ash/index.html', context)
...
def category_list(request, category_id):
    ...
    paginator = Paginator(posts, 3)  # 第二个参数2代表每页显示几个
    page_number = request.GET.get('page')   # http://assas.co/?page=1 (页码)
    page_obj = paginator.get_page(page_number)
    context = {'category': category, 'page_obj': page_obj }
    return render(request, 'blog_ash/list.html', context)
...
def post_detail(request, post_id):
    ...
    return render(request, 'blog_ash/detail.html', context)
...
def archives(request, year, month):
    ...
    return render(request, 'blog_ash/archives_list.html', context )
